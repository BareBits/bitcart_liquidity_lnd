"""Tests for the AUTOMATIC_CHANNEL_CREATION_ENABLED gate.
...
def test_automatic_channel_creation_default_is_false():
    """The shipped default is to delegate to an LSP. If you want the old
    direct-create behavior, opt in via AUTOMATIC_CHANNEL_CREATION_ENABLED=True."""
    import importlib
    import config
    importlib.reload(config)
    assert config.AUTOMATIC_CHANNEL_CREATION_ENABLED is False
...
def test_move_onchain_to_ln_short_circuits_when_disabled(monkeypatch, event_loop):
    ...
    _set(monkeypatch, AUTOMATIC_CHANNEL_CREATION_ENABLED=False)
...
def test_move_onchain_to_ln_runs_when_enabled(monkeypatch, event_loop):
    ...
    _set(monkeypatch, AUTOMATIC_CHANNEL_CREATION_ENABLED=True)
